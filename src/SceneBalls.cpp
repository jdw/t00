...
void SceneBalls::Update() {
	...
	ball[0]->ang -= 0.1;
	ball[0]->pos = dV2(cos(ball[0]->ang) * pScreen->GetSize().x / 3 + tmp_halfScreen.x,
	                   sin(ball[0]->ang) * pScreen->GetSize().y / 3 + tmp_halfScreen.y);
	ball[1]->ang += 0.1;
	ball[1]->pos = dV2(cos(ball[1]->ang) * pScreen->GetSize().x / 2 + tmp_halfScreen.x,
	                   sin(ball[1]->ang) * pScreen->GetSize().y / 2 + tmp_halfScreen.y);
	ball[2]->ang += 0.1;
	ball[2]->pos = dV2(cos(ball[2]->ang) * pScreen->GetSize().x / 7 + tmp_halfScreen.x,
